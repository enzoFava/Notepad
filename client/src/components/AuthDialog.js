...
const AuthDialog = ({ open, onClose, onAuthSuccess }) => {
  ...
  return (
    <Dialog
      ...
      sx={{ '& .MuiDialog-paper': { borderRadius: '10px', padding: '16px', boxShadow: '0 2px 10px rgba(0, 0, 0, 0.2)' }}}
    >
      <DialogTitle sx={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 'bold', fontSize: '1.5em', position: 'relative' }}>
      ...
      <DialogActions sx={{ justifyContent: 'flex-end', padding: '16px' }}>
        <Button
          onClick={() => { onClose(); alert("Notes will not persist the session."); }}
          sx={{
